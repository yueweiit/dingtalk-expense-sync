...
| `approval_expense_dept_split` | 运营支出分部门拆分（CQRS read model，包含工资/社保/办公场地/个税/历史 IT 运维/人工公司分摊） |
...
The sync service extracts actual payment events from approval operation comments. It accepts only authorized users: an explicit amount in the comment takes priority; a single `已支付` comment without an amount uses the form amount component; a `部分支付` comment without an amount remains review-only. Separately, explicitly reviewed historical corrections may use `source_type=manual_confirmed` and `rule_version=manual-confirmed-v1`; these events are also valid confirmed payment facts but are never created by ordinary comment parsing. Events are written to `approval_expense_payment_events` and are idempotent on `business_id + paid_at + source_hash`, so repeated synchronization does not duplicate accounting. Salary, social insurance, housing fund, office space, and individual income tax forms continue through department splits instead of whole-form payment events.
...
## IT Operation and Bonus Compatibility
